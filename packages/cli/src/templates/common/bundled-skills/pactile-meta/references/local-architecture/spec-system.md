...
`.pactile/spec/` is the user's project-specific engineering spec library. Pactile is not about making AI memorize conventions; it injects relevant specs or requires the AI to read them at the right time.
...
`.pactile/config.yaml` can declare packages:
...
```jsonl
{"file": ".pactile/spec/cli/backend/index.md", "reason": "CLI backend conventions"}
{"file": ".pactile/spec/cli/unit-test/conventions.md", "reason": "Test expectations"}
```
...
When the AI learns a new rule during implementation or debugging, it should update `.pactile/spec/` rather than only summarizing it in chat.

## Local Customization Points

| Need | Edit location |
| --- | --- |
| Add a new spec layer | `.pactile/spec/<package>/<layer>/index.md` and corresponding guideline files. |
| Change monorepo spec mapping | `packages` / `default_package` / `spec_scope` in `.pactile/config.yaml`. |
...
| Change when specs should be updated | Phase 3.3 in `.pactile/workflow.md` and the `pactile-update-spec` skill. |

## Boundaries

`.pactile/spec/` is the user's project specification, not a permanent copy of Pactile built-in templates. The AI should encourage the user to update it according to the actual project code instead of treating Pactile default templates as immutable documents.
