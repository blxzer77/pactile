...
This Skill does not implement the Session compiler and does not treat `workflow.md` or `pactile context --mode phase` as runtime SSOT. Run the available CLI yourself. Do not open with a “loaded” probe. No selected task → answer directly.
...
If a **compiled session pack** is already in context, use that pack. Run `pactile context --mode session --json` to refresh it when needed. Do **not** implement or patch the compiler.

Do **not** treat `pactile context --mode phase` (Phase Index) as runtime SSOT. That extractor is a human overview of `workflow.md`, not Kernel truth.

If the output says `Selected task: none`, stop. Show the dashboard and ask the user to choose one route:

- select a task with `pactile task select <task>`
...
```bash
pactile session search --query "<task topic or title>" --json
```
